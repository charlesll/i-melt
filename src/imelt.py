...
class data_loader():
    ...
    def __init__(self, 
        ...
        self.x_elastic_train = torch.FloatTensor(self.scaling(X_elastic_train,X_scaler_mean,X_scaler_std))
        self.y_elastic_train = torch.FloatTensor(y_elastic_train.reshape(-1,1))

        self.x_elastic_valid = torch.FloatTensor(self.scaling(X_elastic_valid,X_scaler_mean,X_scaler_std))
        self.y_elastic_valid = torch.FloatTensor(y_elastic_valid.reshape(-1,1))

        self.x_elastic_test = torch.FloatTensor(self.scaling(X_elastic_test,X_scaler_mean,X_scaler_std))
        self.y_elastic_test = torch.FloatTensor(y_elastic_test.reshape(-1,1))

        # Glass thermal expansion
        self.x_cte_train = torch.FloatTensor(self.scaling(X_cte_train,X_scaler_mean,X_scaler_std))
        self.y_cte_train = torch.FloatTensor(y_cte_train.reshape(-1,1))

        self.x_cte_valid = torch.FloatTensor(self.scaling(X_cte_valid,X_scaler_mean,X_scaler_std))
        self.y_cte_valid = torch.FloatTensor(y_cte_valid.reshape(-1,1))

        self.x_cte_test = torch.FloatTensor(self.scaling(X_cte_test,X_scaler_mean,X_scaler_std))
        self.y_cte_test = torch.FloatTensor(y_cte_test.reshape(-1,1))

        # Glass Abbe Number
        self.x_abbe_train = torch.FloatTensor(self.scaling(X_abbe_train,X_scaler_mean,X_scaler_std))
        self.y_abbe_train = torch.FloatTensor(y_abbe_train.reshape(-1,1))

        self.x_abbe_valid = torch.FloatTensor(self.scaling(X_abbe_valid,X_scaler_mean,X_scaler_std))
        self.y_abbe_valid = torch.FloatTensor(y_abbe_valid.reshape(-1,1))

        self.x_abbe_test = torch.FloatTensor(self.scaling(X_abbe_test,X_scaler_mean,X_scaler_std))
        ...
        self.x_liquidus_train = torch.FloatTensor(self.scaling(X_liquidus_train,X_scaler_mean,X_scaler_std))
        self.y_liquidus_train = torch.FloatTensor(y_liquidus_train.reshape(-1,1))

        self.x_liquidus_valid = torch.FloatTensor(self.scaling(X_liquidus_valid,X_scaler_mean,X_scaler_std))
        self.y_liquidus_valid = torch.FloatTensor(y_liquidus_valid.reshape(-1,1))

        self.x_liquidus_test = torch.FloatTensor(self.scaling(X_liquidus_test,X_scaler_mean,X_scaler_std))
    ...
    def print_data(self):
        ...
        print("")
        print("Number of unique compositions (glass elastic modulus): {}".format(size_total))
        print("Number of unique compositions in training (glass elastic modulus): {}".format(size_train))
        print("Dataset separations are {:.2f} in train, {:.2f} in valid, {:.2f} in test".format(size_train/size_total,
                                                                                    size_valid/size_total,
                                                                                    size_test/size_total))
        ...
        print("Number of unique compositions in training (glass CTE): {}".format(size_train))
        print("Dataset separations are {:.2f} in train, {:.2f} in valid, {:.2f} in test".format(size_train/size_total,
                                                                                    size_valid/size_total,
                                                                                    size_test/size_total))
        ...
        print("")
        print("Number of unique compositions (glass Abbe Number): {}".format(size_total))
        print("Number of unique compositions in training (glass Abbe Number): {}".format(size_train))
        print("Dataset separations are {:.2f} in train, {:.2f} in valid, {:.2f} in test".format(size_train/size_total,
                                                                                    size_valid/size_total,
                                                                                    size_test/size_total))
        ...
        print("")
        print("Number of unique compositions (Liquidus temperature): {}".format(size_total))
        print("Number of unique compositions in training (Liquidus temperature): {}".format(size_train))
        print("Dataset separations are {:.2f} in train, {:.2f} in valid, {:.2f} in test".format(size_train/size_total,
                                                                                    size_valid/size_total,
                                                                                    size_test/size_total))
...
class model(torch.nn.Module):
    ...
    def __init__(self, input_size, hidden_size = 300, num_layers = 4, nb_channels_raman = 800, 
        ...
        ###
        if self.shape == "transformer":
            self.out_thermo = torch.nn.Linear(self.hidden_size*self.input_size, 34) # Linear output, 22 without Cp
            self.out_raman = torch.nn.Linear(self.hidden_size*self.input_size, self.nb_channels_raman) # Linear output
        else:
            self.out_thermo = torch.nn.Linear(self.hidden_size, 34) # Linear output, 22 without Cp
    ...
    def output_bias_init(self):
        ...
        self.out_thermo.bias = torch.nn.Parameter(data=torch.tensor([np.log(1000.),np.log(8.), # Tg, ScTg
                                                                     ...
                                                                     np.log(1585.) # liquidus temperature
                                                                     ]))
...
def training(neuralmodel, ds, 
    ...
    slices_x_liquidus_train = [ds.x_liquidus_train[i::nb_folds] for i in range(nb_folds)]
    slices_y_liquidus_train = [ds.y_liquidus_train[i::nb_folds] for i in range(nb_folds)]
