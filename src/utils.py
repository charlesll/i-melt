...
def descriptors(X):
    ...
    # DOI 10.4401/ag-3221
    # Table 1
    X["optbas"] = (0.48*X.sio2 + 0.59*X.al2o3 + 
                   1.15*X.na2o + 1.36*X.k2o + 
                   0.78*X.mgo + 0.99*X.cao)
    ...
    X["al_m"] = X.al2o3 / (X.al2o3+X.loc[:,["na2o","k2o","mgo","cao"]].sum(axis=1))
...
def chimie_control(data):
    ...
    sum_oxides = datalist["sio2"]+datalist["al2o3"]+datalist["na2o"]+datalist["k2o"]+datalist["mgo"]+datalist["cao"]
...
def class_data(chemical_set):
    ...
    chemical_set = chemical_set.loc[:,["sio2","al2o3","na2o","k2o","mgo","cao"]].values
    ...
    class_of_data[(chemical_set[:,[0,1]] >= 0).all(axis=1)&
                (chemical_set[:,[2,3,4,5]] == 0).all(axis=1)] = 1

    # sio2-na2o, class 2
    class_of_data[(chemical_set[:,[0,2]] > 0).all(axis=1)&
                (chemical_set[:,[1,3,4,5]] == 0).all(axis=1)] = 2

    # sio2-k2o, class 3
    class_of_data[(chemical_set[:,[0,3]] > 0).all(axis=1)&
                (chemical_set[:,[1,2,4,5]] == 0).all(axis=1)] = 3

    # sio2-mgo, class 4
    class_of_data[(chemical_set[:,[0,4]] > 0).all(axis=1)&
                (chemical_set[:,[1,2,3,5]] == 0).all(axis=1)] = 4

    # sio2-cao, class 5
    class_of_data[(chemical_set[:,[0,5]] > 0).all(axis=1)&
                (chemical_set[:,[1,2,3,4]] == 0).all(axis=1)] = 5

    # sio2-alkali ternary, class 6
    class_of_data[(chemical_set[:,[0,2,3]] > 0).all(axis=1)&
                (chemical_set[:,[1,4,5]] == 0).all(axis=1)] = 6

    # sio2-alkaline-earth ternary, class 7
    class_of_data[(chemical_set[:,[0,4,5]] > 0).all(axis=1)&
                (chemical_set[:,[1,4,5]] == 0).all(axis=1)] = 7

    # ternaries sio2- MO - M2O, all in class 8
    class_of_data[(chemical_set[:,[0,2,4]] > 0).all(axis=1)&
                (chemical_set[:,[1,3,5]] == 0).all(axis=1)] = 8
                
    class_of_data[(chemical_set[:,[0,2,5]] > 0).all(axis=1)&
                (chemical_set[:,[1,3,4]] == 0).all(axis=1)] = 8

    class_of_data[(chemical_set[:,[0,3,4]] > 0).all(axis=1)&
                (chemical_set[:,[1,2,5]] == 0).all(axis=1)] = 8

    class_of_data[(chemical_set[:,[0,3,5]] > 0).all(axis=1)&
                (chemical_set[:,[1,2,4]] == 0).all(axis=1)] = 8

    # al2o3-cao, class 9
    class_of_data[(chemical_set[:,[1,5]] > 0).all(axis=1)&
                (chemical_set[:,[0,2,3,4]] == 0).all(axis=1)] = 9

    # sio2-al2o3-na2o, class 10
    class_of_data[(chemical_set[:,[0,1,2]] > 0).all(axis=1)&
                (chemical_set[:,[3,4,5]] == 0).all(axis=1)] = 10

    # sio2-al2o3-k2o, class 11
    class_of_data[(chemical_set[:,[0,1,3]] > 0).all(axis=1)&
                (chemical_set[:,[2,4,5]] == 0).all(axis=1)] = 11

    # sio2-al2o3-na2o-k2o, class 12
    class_of_data[(chemical_set[:,[0,1,2,3]] > 0).all(axis=1)&
                (chemical_set[:,[4,5]] == 0).all(axis=1)] = 12

    # sio2-al2o3-mgo, class 13
    class_of_data[(chemical_set[:,[0,1,4]] > 0).all(axis=1)&
                (chemical_set[:,[2,3,5]] == 0).all(axis=1)] = 13

    # sio2-al2o3-cao, class 14
    class_of_data[(chemical_set[:,[0,1,5]] > 0).all(axis=1)&
                (chemical_set[:,[2,3,4]] == 0).all(axis=1)] = 14

    # sio2-al2o3-mgo-cao, class 15
    class_of_data[(chemical_set[:,[0,1,4,5]] > 0).all(axis=1)&
                (chemical_set[:,[2,3]] == 0).all(axis=1)] = 15
    
    return class_of_data

def preprocess_raman(my_liste, path_spectra = "./data/raman/", generate_figures = False, save_path="./figures/datasets/raman/"):
    ...
    for i in range(nb_exp):
        ...
        if file_extension == ".txt" or file_extension == ".TXT":
            ...
            sp = np.genfromtxt(path_spectra+my_liste.loc[i,"nom"],skip_header=1,delimiter=",")
        else:
        ...
        if my_liste.raw[i] == "yes":
            _, y_long, _ = rp.tlcorrection(sp[:,0], sp[:,1]-np.min(sp[sp[:,0]>600,1])+1e-16, 23.0,
        ...
        y_resampled[x<np.max(sp[:,0])] = rp.resample(sp[:,0], y_smo, x[x<np.max(sp[:,0])], fill_value="extrapolate")
        p_ = np.polyfit(sp[sp[:,0]>(np.max(sp[:,0])-30),0],y_smo[sp[:,0]>(np.max(sp[:,0])-30)],1)
